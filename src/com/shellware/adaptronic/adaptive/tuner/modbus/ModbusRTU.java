/*
 *   Copyright 2012 Shell M. Shrader
 *
 *  Licensed under the Apache License, Version 2.0 (the "License");
 *  you may not use this file except in compliance with the License.
 *  You may obtain a copy of the License at
 *
 *     http://www.apache.org/licenses/LICENSE-2.0
 *
 *  Unless required by applicable law or agreed to in writing, software
 *  distributed under the License is distributed on an "AS IS" BASIS,
 *  WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 *  See the License for the specific language governing permissions and
 *  limitations under the License.
 *
 */
package com.shellware.adaptronic.adaptive.tuner.modbus;

import java.nio.ByteBuffer;
import java.nio.ByteOrder;

public class ModbusRTU {

    public static final byte[] getRegister(final byte slaveAddr, final byte function, final short register) {
    	return getRegister(slaveAddr, function, register, (short) 1);
    }
    public static final byte[] getRegister(final byte slaveAddr, final byte function, final short register, final short count) {	
    	// convert our register into a 2 byte array -- (16 bit short integer)
    	ByteBuffer rr = ByteBuffer.allocate(2);
    	rr.order(ByteOrder.BIG_ENDIAN);
    	rr.putShort(register);
    	byte[] r = rr.array();
    	
    	// convert our count into a 2 byte array - (16 bit short integer)
    	ByteBuffer cc = ByteBuffer.allocate(2);
    	cc.order(ByteOrder.BIG_ENDIAN);
    	cc.putShort(count);
    	byte[] c = cc.array();
    	
    	final byte[] message = { slaveAddr, function, r[0], r[1], c[0], c[1], 0x00, 0x00 };
    	int[] crc = ModbusRTU.getCRC(message, message.length - 2);
    	
    	message[message.length - 2] = (byte) crc[0];
    	message[message.length - 1] = (byte) crc[1];	
    	
    	return message;
    }
    
	public static final boolean validCRC(final String[] msg, final int length) {

		// discard short messages
		if (msg.length < length) return false;
		
		byte[] buf = new byte[512];

		for (int x = 0; x < length - 2; x++) {
			buf[x] = (byte) Integer.parseInt(msg[x], 16);
		}

		final int[] crc = ModbusRTU.getCRC(buf, length - 2);
		final int crc0 = Integer.parseInt(msg[length - 2], 16);
		final int crc1 = Integer.parseInt(msg[length - 1], 16);

		final boolean res = (crc[0] == crc0 && crc[1] == crc1);

		return res;
	}

// !!!!!!!!!!!!! NOTE !!!!!!!!!!!!!
//  GNU GPL  applies to following
//  code only.
// !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
	
/***
 * Java Modbus Scanner Copyright (c) 2009, Andrzej Sokulski andrzej@modbus.pl -
 * www.modbus.pl All rights reserved.
 * 
 * 
 * This software is free you can redistribute it and/or modify it under the
 * terms of the GNU Lesser General Public License as published by the Free
 * Software Foundation; either version 2.1 of the License, or (at your option)
 * any later version.
 * 
 * This software is distributed in the hope that it will be useful, but WITHOUT
 * ANY WARRANTY; without even the implied warranty of MERCHANTABILITY or FITNESS
 * FOR A PARTICULAR PURPOSE. See the GNU Lesser General Public License for more
 * details. In no event shall the regents or contributors be liable for any
 * direct, indirect, incidential, special, exemplary, or consequential damages.
 * 
 * If you want to remove the copyright notice, please donate me
 * (www.modbus.pl/donate.html).
 */
	public static final int[] getCRC(byte[] buffer, int length) {

		int[] crc = { 0xFF, 0xFF };
		int nextVal = 0;
		int index;
		if (length <= buffer.length)

			for (int i = 0; i < length; i++) {
				nextVal = 0xFF & ((int) buffer[i]);
				index = crc[0] ^ nextVal;
				crc[0] = crc[1] ^ CRCHigh[index];
				crc[1] = CRCLow[index];
			}

		return crc;
	}

	/* Hogh Table of CRC values */
	private final static short[] CRCHigh = {  };

	/* Low Table of CRC values */
	private final static short[] CRCLow = {  };

}